    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        try {
          ...
          const modTitle = await moderateText(titleStr);
          if (modTitle.isToxic) {
            setFiles(prev => prev.map(file => 
              file.id === fileData.id 
                ? { ...file, status: 'error', error: `Title blocked: ${modTitle.reasons.join(', ')}`, progress: 0 }
                : file
            ));
            continue;
          }
          if (fileData.description.trim()) {
            const modDesc = await moderateText(fileData.description.trim());
            if (modDesc.isToxic) {
              setFiles(prev => prev.map(file => 
                file.id === fileData.id 
                  ? { ...file, status: 'error', error: `Description blocked: ${modDesc.reasons.join(', ')}`, progress: 0 }
                  : file
              ));
              continue;
            }
          ...
          console.warn('AI moderation unavailable in bulk upload, using local checks.', e);
        }
        ...
        if (isImageFile(fileData.file.type)) {
          try {
            ...
            if (newHash && batchHashesRef.current.length > 0) {
              ...
              if (isDuplicate) {
                dupWarningsRef.current.push(`${fileData.file.name}: potential duplicate (similarity ${similarity})`);
              }
            }
          } catch (e) {
