...
import { getSignedUrl } from '../lib/storageManager';
...
export default function MediaDetailModal({ media, isOpen, onClose }: MediaDetailModalProps) {
 ...
 const EditableTitle: React.FC<{ media: Media; canEdit: boolean }> = ({ media, canEdit }) => {
   ...
   const saveTitle = async () => {
     ...
     try {
       setSaving(true);
       // Local text safety check for title
                        ...
                        <button
                          onClick={async () => {
                            try {
                              ...
                              // Check global limit before allowing caption generation
                              if (user) {
                                const accessCheck = await checkCaptionAccess(user.id);
                                
                                if (!accessCheck.allowed) {
                                  toast.error(accessCheck.reason || 'AI Captioner access denied');
                                  setGeneratingCaption(false);
                                  return;
                                }
                                ...
                                if (accessCheck.isAdmin) {
                                  toast.info('🎨 Admin: Unlimited AI captions');
                                } else if (accessCheck.remaining !== undefined && accessCheck.remaining !== Infinity) {
                                  if (accessCheck.remaining <= 2) {
                                    toast.warning(`⚠️ Only ${accessCheck.remaining} AI captions left today globally! (${accessCheck.globalUsed}/${accessCheck.globalLimit} used)`);
                                  } else {
                                    toast.info(`✨ ${accessCheck.remaining} AI captions remaining today (${accessCheck.globalUsed}/${accessCheck.globalLimit} used globally)`);
                                  }
                                }
                              }
                              ...
                              try {
                                ...
                                // Record usage after successful AI caption
                                if (user) {
